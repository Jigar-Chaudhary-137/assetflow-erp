...
const normalizeAllocation = (alloc) => {
  ...
  const assetName =
    alloc.assetId && typeof alloc.assetId === "object"
      ? alloc.assetId.name
  ...
  const employeeName =
    alloc.employeeId && typeof alloc.employeeId === "object"
  ...
  const allocatedBy =
    alloc.allocatedById && typeof alloc.allocatedById === "object"
  ...
  return {
    ...
    allocatedDate: alloc.allocatedDate
      ? new Date(alloc.allocatedDate).toISOString().split("T")[0]
      : alloc.createdAt
        ? new Date(alloc.createdAt).toISOString().split("T")[0]
        : "",
    dueDate: alloc.expectedReturnDate
      ? new Date(alloc.expectedReturnDate).toISOString().split("T")[0]
      : "",
    returnedDate: alloc.actualReturnDate
      ? new Date(alloc.actualReturnDate).toISOString().split("T")[0]
      : null,
    status,
...
export const allocationService = {
  ...
  returnAsset: async (allocationId, notes = "") => {
    const res = await api.post(`/allocations/${allocationId}/return`, {
      notes,
    });
    return normalizeAllocation(res.data.data);
  },
  ...
  approveTransfer: async (allocationId) => {
    const res = await api.patch(
      `/allocations/${allocationId}/transfer/approve`,
    );
